...
{
	char ch = 0;
	return vsnprintf(&ch, 0, format, args);
}
...
{
	wchar_t ch = 0;
	return vswprintf(&ch, 0, format, args);
}

template<>
inline int CryStringT<char>::_vsnpf(value_type* buf, int cnt, const_str format, va_list args)
{
	// we have to be careful, since MSVC's _vsnwprintf() doesn't null-terminate
	//  the string on overflow...you just get all (cnt) chars of (buf) filled.
	// To avoid losing a character (or writing a null past the end), we have
	//  to format to a temp buffer and copy. So lame.
	value_type *tmpbuf = new value_type[cnt+1];
	const int retval = vsnprintf(tmpbuf, cnt+1, format, args);
	const int cpy = (retval >= cnt) ? cnt : (retval + 1);
	memcpy(buf, tmpbuf, cpy * sizeof (value_type));
	delete[] tmpbuf;
	return (retval >= cnt) ? -1 : retval;
}
...
inline int CryStringT<wchar_t>::_vsnpf(value_type* buf, int cnt, const_str format, va_list args)
{
	...
	//  to format to a temp buffer and copy. So lame.
	value_type *tmpbuf = new value_type[cnt+1];
	const int retval = vswprintf(tmpbuf, cnt+1, format, args);
	const int cpy = (retval >= cnt) ? cnt : (retval + 1);
	memcpy(buf, tmpbuf, cpy * sizeof (value_type));
	delete[] tmpbuf;
	return (retval >= cnt) ? -1 : retval;
