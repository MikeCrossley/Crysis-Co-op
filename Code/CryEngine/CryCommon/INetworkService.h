...
struct SFileDownloadParameters
{
	...
	uint64 md5;								// md5 checksum of the file
	int fileSize;
...
enum EFileDownloadError
{
	eFDE_NoError = 0,           // Successfully retrieved file OR still in progress
	eFDE_OutOfMemory,           // A memory allocation failed.
	eFDE_BufferOverflow,        // The user-supplied buffer was too small to hold the file.
	eFDE_ParseURLFailed,        // There was an error parsing the URL.
	eFDE_HostLookupFailed,      // Failed looking up the hostname.
	eFDE_SocketFailed,          // Failed to create/initialize/read/write a socket.
	eFDE_ConnectFailed,         // Failed connecting to the http server.
	eFDE_BadResponse,           // Error understanding a response from the server.
	eFDE_RequestRejected,       // The request has been rejected by the server.
	eFDE_Unauthorized,          // Not authorized to get the file.
	eFDE_Forbidden,             // The server has refused to send the file.
	eFDE_FileNotFound,          // Failed to find the file on the server.
	eFDE_ServerError,           // The server has encountered an internal error.
	eFDE_FileWriteFailed,       // An error occured writing to the local file 
	eFDE_FileIncomplete,        // Download started but was interrupted.  Only reported if file size is known.
	eFDE_FileTooBig,            // The file is to big to be downloaded (size exceeds range of interal data types)
	eFDE_EncryptionError,       // Error with encryption engine.
	eFDE_RequestCancelled,      // User requested cancel and/or graceful close.

	eFDE_LastError,							// so game code can add more error codes.
};
...
struct IFileDownloader : public INetworkInterface
{
	...
	// get error code (might be 0 if successful / in progress)
	virtual int GetDownloadError() const = 0;
};
