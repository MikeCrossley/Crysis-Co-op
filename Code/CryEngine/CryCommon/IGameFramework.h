...
// stored if connection fails due to map errors so we can reconnect after downloading the map
struct SServerConnectionInfo
{
	string serverAddr;			// either "192.168.0.1" or "pc149"
	string serverPort;	
	string levelName;				// "Shore" or "Multiplayer/PS/Shore"
	string levelDownloadURL;// "http://www.server.com/filename.zip"
	uint64 levelChecksum;		// md5 from LevelSystem
};

struct SModInfo
{
	const char *m_name, *m_version, *m_description, *m_url, *m_screenshot, *m_displayname, *m_team;
};
...
struct IGameFramework
{
  ...
  virtual bool SaveServerConfig(const char* path) = 0;
	...
	// Description:
	//	store server info for later connection attempts
	virtual void StoreServerInfo(SServerConnectionInfo& info) = 0;

	// Description:
	//	retrieve previously stored info
	virtual SServerConnectionInfo* GetStoredServerInfo() = 0;

	// Description:
	virtual void ClearStoredServerInfo() = 0;

	// Description:
	//	read CD key from registry and set it in the network
	virtual void ReadCDKey() = 0;

	// Description:
	//	save a user-entered cd key into the registry
	virtual void SaveCDKey(const char* cdKey) = 0;

	// Description:
	//	returns true if a Cd key was successfully read at app startup
	virtual bool WasCDKeyReadSuccessfully() = 0;
};
