...
void __finddata64_t::CopyFoundData(const char *rMatchedFileName)
{
	...
	snprintf(filename, sizeof(filename), "%s/%s", m_DirectoryName, name);
#if 0
...
// !!! FIXME: ugh, this code is such a mess! Throw this all out and
// !!! FIXME:  replace it with a few lines that work everywhere.
#if !defined(USE_HDD0)
...
#else
	#if defined(_DEBUG)
		//check and message here to remove conversion later
//		if(strchr(filename, '\\'))
//			printf("fopen: found '\\' in file name \"%s\"\n",filename);
	#endif
	return ::fopen(ConvertFileName(filename), mode);
#endif//!defined(USE_HDD0)
	}
