...
#define SERVER_DEFAULT_PORT 64100
#define SERVER_DEFAULT_PORT_STRING "64100"
...
enum EDisconnectionCause
{
  ...
  eDC_UserRequested,
	...
	eDC_MapVersion,
	// client is not running punkbuster, but server is
	eDC_PunkBusterRequired,
	// server has changed map since we downloaded the last one
	eDC_ServerMapChanged_NewOneNotFound,
...
struct INetwork
{
  ...
  virtual void SetCDKey(const char*) = 0;
	...
	// Description:
	//		Return the CD key read from the registry on app startup.
	virtual const char* GetCDKey() = 0;

	// Description:
	//		Returns true if a cdkey has been set, false otherwise.
	//			NB does *not* do any validation checking
	virtual bool HaveCDKey() = 0;
...
struct INetChannel : public INetMessageSink
{
	...
	// Description:
	//		Disconnect this channel via Punkbuster (if possible)
	virtual void DisconnectThroughPB(const char *reason, const char *log) = 0;
	// Description:
