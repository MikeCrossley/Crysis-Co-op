...
//	virtual bool IsOutOfDate(const char * szCompiledName, const char * szMasterFile)=0;
	...
	// Attempts to extract a pak (zip) file into it's current folder.
	//	folder structure is preserved. Returns 0 for fail, +ve for success (for now).
	virtual int ExtractPack(const char* fileName) = 0;
};
