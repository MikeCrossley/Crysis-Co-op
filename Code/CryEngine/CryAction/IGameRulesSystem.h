...
enum ETextMessageType
{
	...
	eTextMessageServer,
	eTextMessageBig,	// bigger message display using the 'you killed...' message space (new for patch 3)
};
...
struct IGameRules : public IGameObjectExtension
{
  ...
  virtual const char *GetHitType(int id) const = 0;
	...
	virtual void OnVehicleDestroyed(EntityId id, EntityId shooterId) = 0;
	...
	virtual void ScheduleEntityRespawn(EntityId entityId, bool unique, bool spatialcheck, float timer) = 0;
