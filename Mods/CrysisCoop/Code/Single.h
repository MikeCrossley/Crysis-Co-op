  ...
  struct SDustParams
  {
	...
	struct SSpreadParams
	{
		...
		void Reset(const IItemParamsNode *params=0, bool defaultInit=true)
		{
			...
			ResetValue(spread_stand_to_crouch_m, 1.5f);
			ResetValue(spread_to_prone_m, 5.0f);
		}
		...
		float spread_stand_to_crouch_m;
		float spread_to_prone_m;
...
public:
  ...
  //~IFireMode
	...
	virtual Vec3 ApplySpread(const Vec3 &dir, float spread) const;
...
protected:
	...
	bool InternalShoot(IEntityClass* spawn_ammo, bool resetAnimation, bool autoreload = true , bool noSound = false);
	void InternalNetShootEx(IEntityClass* spawn_ammo, const Vec3 &pos, const Vec3 &dir, const Vec3 &vel, const Vec3 &hit, float extra, int predictionHandle);
