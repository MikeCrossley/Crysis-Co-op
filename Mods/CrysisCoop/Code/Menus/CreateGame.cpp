...
void CMultiPlayerMenu::SCreateGame::StartServer()
{
  ...
  if(m_anticheat)
	...
	if(ICVar* var = pConsole->GetCVar("hud_creategame_pb_server"))
		var->Set(m_anticheat?1:0);
...
void CMultiPlayerMenu::SCreateGame::GetLevelRotation()
{
      ...
      case eMO_timelimitMinutes:
        ...
        break;
			case eMO_killlimit:
				...
				break;
			case eMO_scorelimit:
				scorelimit = IsTrue(valueArray[i]);
				break;
			case eMO_score:
				score = atoi(valueArray[i]);
				break;
    ...
    rotation->AddSetting(levelidx,string().Format("g_timelimit %d",timelimit?time:0));
		...
		rotation->AddSetting(levelidx,string().Format("g_scorelimit %d",scorelimit?score:0));
...
void CMultiPlayerMenu::SCreateGame::SetGlobalSettings()
{
  ...
  if(ICVar* v = gEnv->pConsole->GetCVar("g_friendlyfireratio"))
  {
	...
	if(ICVar* v = gEnv->pConsole->GetCVar("sv_requireinputdevice"))
	{
		m_controllersOnly = !stricmp(v->GetString(),"gamepad");
		keyArray.push_back(VALUE_BY_KEY(eCGO_controllersOnly,gCreateGameOptions));
		valueArray.push_back(m_controllersOnly?"1":"0");
	}

	if(ICVar* v = gEnv->pConsole->GetCVar("hud_creategame_pb_server"))
	{
		m_anticheat = v->GetIVal()!=0;
		keyArray.push_back(VALUE_BY_KEY(eCGO_anticheat,gCreateGameOptions));
		valueArray.push_back(m_anticheat?"1":"0");
	}
		...
		if(rot && rot->GetLength()>0)
		{
			...
			do 
			{
				...
				for(int i=0;i<rot->GetNextSettingsNum();++i)
				{
					...
					if(!var.compareNoCase("g_scorelimit"))
					{
						int f = atoi(val);
						keyArray.push_back(VALUE_BY_KEY(eMO_scorelimit,gMapOptions));
						valueArray.push_back(f==0?"0":"1");

						cache.push_back(val.c_str());
						keyArray.push_back(VALUE_BY_KEY(eMO_score,gMapOptions));
						valueArray.push_back(cache.back());
					}
