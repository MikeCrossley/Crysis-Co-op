...
protected:
  ...
  virtual bool CanIgnore(EChatCategory cat, int id, const char* nick);
	...
	virtual void OnAddFavoriteByIP(uint ip, ushort port);
...
void CMultiPlayerMenu::CUI::OnAddFavoriteByIP(uint ip, ushort port)
{
	m_menu->m_favouriteServers.push_back(SStoredServer(ip,port));
	m_menu->m_profile->AddFavoriteServer(ip,port);
}
