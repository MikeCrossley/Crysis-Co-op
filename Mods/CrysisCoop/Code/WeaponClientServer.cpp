...
void CWeapon::NetMeleeAttack(bool weaponMelee, const Vec3 &pos, const Vec3 &dir)
{
	if (weaponMelee && m_melee)
	{
		...
		if (IsServer())
			m_pGameplayRecorder->Event(GetOwner(), GameplayEvent(eGE_WeaponMelee, 0, 0, ENTITY_ID_TO_POINTER(GetEntityId())));
	}
	else if (m_fm)
	{
...
IMPLEMENT_RMI(CWeapon, SvRequestZoom)
{
	...
	if (ok)
	{
		...
		if (params.fov<0.99f)
			event=eGE_ZoomedIn;
		m_pGameplayRecorder->Event(GetOwner(), GameplayEvent(event, 0, 0, ENTITY_ID_TO_POINTER(GetEntityId())));
	}
...
IMPLEMENT_RMI(CWeapon, SvRequestReload)
{
	...
	if (ok)
	{
		...
		m_pGameplayRecorder->Event(GetOwner(), GameplayEvent(eGE_WeaponReload, 0, 0, ENTITY_ID_TO_POINTER(GetEntityId())));
	}
