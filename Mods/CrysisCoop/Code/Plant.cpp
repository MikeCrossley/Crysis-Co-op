...
struct CPlant::StartPlantAction
{
	...
	void execute(CItem *_this)
	{
		...
		bool selectNextWeaponOrDetonator = pPlant->m_pWeapon->OutOfAmmo(false);
		if(!selectNextWeaponOrDetonator)
		{
			// get number of projectiles so far planted. (c4 only). If the maximum allowed number has
			//	been planted, switch to detonator.
			if(!pPlant->m_plantparams.simple)
			{
				CPlayer* pPlayer = static_cast<CPlayer*>(pPlant->m_pWeapon->GetOwnerActor());
				if(pPlayer && pPlayer->GetExplosiveCount(eET_C4) == g_pGameCVars->g_explosiveLimits[eET_C4])
				{
					selectNextWeaponOrDetonator = true;
				}
			}
		}
