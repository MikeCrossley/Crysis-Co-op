...
bool CActor::PickUpItem(EntityId itemId, bool sound, bool ignoreOffhand)
{
	...
	if (gEnv->bServer || (pItem->GetEntity()->GetFlags()&(ENTITY_FLAG_CLIENT_ONLY|ENTITY_FLAG_SERVER_ONLY)))
	{
		...
		m_pGameplayRecorder->Event(GetEntity(), GameplayEvent(eGE_ItemPickedUp, 0, 0, ENTITY_ID_TO_POINTER(pItem->GetEntityId())));
	}
	else
...
bool CActor::DropItem(EntityId itemId, float impulseScale, bool selectNext, bool bydeath)
{
	...
	if (pItem->CanDrop())
	{
		...
		if (gEnv->bServer)
		{
			...
			if (!bydeath)
				m_pGameplayRecorder->Event(GetEntity(), GameplayEvent(eGE_ItemDropped, 0, 0, ENTITY_ID_TO_POINTER(itemId)));
		}
		else
	...
	if(gEnv->bServer && GetWeaponAttachmentManager())
	{
		...
		while(it!=weaponList.end())
		{
			...
			if(pItemBack)
			{
				...
				m_pGameplayRecorder->Event(GetEntity(), GameplayEvent(eGE_ItemDropped, 0, 0, ENTITY_ID_TO_POINTER(pItemBack->GetEntityId())));
			}
