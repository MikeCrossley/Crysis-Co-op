...
CGunTurret::ETargetClass CGunTurret::GetTargetClass(IEntity* pTarget)const
{
	...
	//Vehicles only check
	if (m_turretparams.vehicles_only && !pVehicle)
		return eTC_NotATarget;

	if(m_turretparams.air_vehicles_only && (!pVehicle || !pVehicle->GetMovement() || pVehicle->GetMovement()->GetMovementType() != IVehicleMovement::eVMT_Air))
		return eTC_NotATarget;
...
{
	m_nextUpdateTimer -= ctx.fFrameTime;
	if(m_nextUpdateTimer < 0.0f)
	{
		m_randoms[eRV_UpdateTimer].New();
		m_nextUpdateTimer = m_randoms[eRV_UpdateTimer].Val();
	}
	else
		return;
