...
function CoopScout_x:ResetAnimation()
	self:StopAnimation(0,3);
end
...
function CoopScout_x:DoShootWeapon()
	self:PushEvent("moac_attack");
	if (self.voiceTable) then
	end
end
	
function CoopScout_x:DoShootWeaponLong()
	self:PushEvent("moac_attack_long");
	if (self.voiceTable) then
	end
end
	
function CoopScout_x:DoInvestigate()
...
--	Script.SetTimer(3000, CoopScout_x.OnDeath, self);				
...
function CoopScout_x:ScriptEvent(event,value,str)		
...
function CoopScout_x:GetDamageMultiplier(hit)
...
function CoopScout_x.Client:OnHit(hit, remote)
...
function CoopScout_x:GrabEntity(entityname)
...
function CoopScout_x:InitiateAutoDestruction()
	if ( self.iAutoDestructTimer == 0 ) then
	  self.iAutoDestructTimer = 1;
	  Script.SetTimerForFunction( self.warmupAutoDestructionTime*1000, "CoopScout_x.WarmupAutoDestruct",self);
	end
end

function CoopScout_x:WarmupAutoDestruct()			
	...
	if ( self.iAutoDestructTimer == 1 ) then
		...
		if ( self.waterCrash ~= nil) then
			...
			if ( self.waterCrash > 3000 ) then
				...
				Script.SetTimerForFunction(self.autoDestructionTime*1000,"CoopScout_x.AutoDestruct",self);
			else
			  Script.SetTimerForFunction( 200, "CoopScout_x.WarmupAutoDestruct",self);
			end
...
function CoopScout_x:AutoDestruct()
...
function CoopScout_x:ReApplyImpulse()
...
function CoopScout_x:PushEntities(hit)
...
function CoopScout_x.Client:OnCollision(hit)
...
-----------------------------------------------------------------------------------------------------
function CoopScout_x:MakeParalyzed( dir, strength )
...
function CoopScout_x.OnDeath( entity )
--	AI.LogEvent(" >>>> CoopScout_x.OnDeath "..entity:GetName());
...
function CoopScout_x:GrabObject(object, query)
...
------------------------------------------------------------------------------
function CoopScout_x:EnableSearchBeam(bEnable)  
  self.actor:CreateCodeEvent({event="searchBeam", enable=bEnable});  
end

------------------------------------------------------------------------------
function CoopScout_x:SetSearchBeamDir(worldDir)  
  self.actor:SetSearchBeam(worldDir);    
end


function CoopScout_x:GrabMultiple()
	...
	--System.Log("CoopScout_x:GrabMultiple() for "..self:GetName().."/"..numGrabbed)
...
function CoopScout_x:DropObject(throw,throwVec,throwDelay)
...
function CoopScout_x:DropMultiple(throw, throwVec, throwDelay)
...
function CoopScout_x:OnSave(save)
...
function CoopScout_x:OnLoad(save)
